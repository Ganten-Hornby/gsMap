"""
Classes for LD Score regression.

These classes implement LD Score regression for estimating heritability
and genetic correlation from GWAS summary statistics.
"""
...
from gsMap.utils.irwls import IRWLS
...
class Hsq(LD_Score_Regression):
    """
    LD Score regression for estimating heritability from GWAS summary statistics.
    ...
    def _update_func(self, x, ref_ld_tot, w_ld, N, M, Nbar, intercept=None, ii=None):
        ...
        hsq = M * x[0][0] / Nbar
        if intercept is None:
            intercept = max(x[0][1], 1.0)  # needed for numerical stability
        else:
...
class Gencov(LD_Score_Regression):
    """
    LD Score regression for estimating genetic covariance from GWAS summary statistics.
    
    Parameters
    ----------
    z1, z2 : np.ndarray
        Z-scores from the first and second trait.
    x : np.ndarray
        LD Scores.
    w : np.ndarray
        Regression weights.
    ...
    M : np.ndarray
        Number of SNPs used for LD Score estimation.
    hsq1, hsq2 : float
        Heritability estimates for each trait.
    intercept_hsq1, intercept_hsq2 : float
        Intercept estimates from individual trait regressions.
    n_blocks : int
        Number of blocks for block jackknife.
    intercept_gencov : float or None
        Intercept constraint for genetic covariance.
    
    Attributes
    ----------
    p, z : float
        P-value and Z-score for genetic correlation test.
    mean_z1z2 : float
        Mean product of Z-scores.
    """
    ...
    def weights(cls, ld, w_ld, N1, N2, M, h1, h2, rho_g, intercept_gencov=None,
        ...
        except FloatingPointError:  # numerical stability
